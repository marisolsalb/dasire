...
```{r}
setwd("/nfs/home/students/chit/DASiRe/serverside/R/examples")
...
library(reshape2)
library(dplyr)
sample_info <- read.table("meta.txt",header = TRUE,sep = "\t")
...
sf_list <- read.table(file = "splicing_factors_list.txt", header = T,sep = "\t",fill = TRUE)

# import reference ENCODE datasets 
ENCODE_metadata <- read.table("encode_bedNarrowPeak_files/metadata.tsv",sep="\t",header = TRUE)
...
for (accession in 1:nrow(input_chips)) {
  all.chips[[input_chips$Experiment.target[accession]]] <- readPeakFile(peakfile = paste0("encode_bedNarrowPeak_files/",input_chips$File.accession[accession],".bed.gz"))
}
