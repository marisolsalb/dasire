...
ui <- dashboardPage(
    preloader = list(html = tagList(spin_flower(), "Please wait..."), color = "#66c0c8"),
    ...
    dashboardBody(
      ...
      shinyDashboardThemes(
        ...
        tabItems(
                        ...
                        fluidRow(
                            column(12,
                                   box(
                                       ...
                                       div(#h4(HTML("<b>Example data</b>"),span(shiny::icon("info-circle"), id = "rna_ex")),
                                           #tippy::tippy_this(elementId = "rna_ex",tooltip = "a to use our example data and press the upload button.",placement = "right"),
                            ...
                            column(12,
                                   box(
                                     ...
                                     actionButton("boris", label="Start!")
                                   )
                        )
                    )
            ...
            tabItem(
                tabName="chip_qc",
                fluidRow(column(12,box(width=NULL,
...
server <- function(input, output, session) {
    ...
    boris <- reactiveValues(boris=FALSE)
    custom <- reactiveValues(custom=FALSE)
    
    observeEvent(input$renderimport_rna, 
                 isolate({custom$custom=TRUE
                 boris$boris=FALSE}))
    observeEvent(input$boris, 
                 isolate({custom$custom=FALSE 
                   boris$boris=TRUE
                   }))
    ...
    global <- reactiveValues(datapath = "No folder chosen.")
    chosen_dir <- reactive(input$dir)
    ...
    observeEvent(ignoreNULL = TRUE,
                 ...
                 handlerExpr = {
                   if (!"path" %in% names(chosen_dir())) return()
                   home <- normalizePath("~")
                   global$datapath <-
                     file.path(home, paste(unlist(chosen_dir()$path[-1]), collapse = .Platform$file.sep))
                 })
    
    output$chosendir <- renderText({global$datapath})
    ...
    output$doc_page <- renderUI({
        #HTML(markdown::markdownToHTML(knitr::knit("documentation/tutorial.Rmd", quiet = TRUE)))
    ...
    output$rnaseq_page <- renderMenu({
        ...
        if(boris$boris==FALSE & custom$custom==FALSE) return()
            ...
            tabBox(
                ...
                tabPanel("PCA",
                         fluidRow(column(6,div(
                             ...
                             div(
                                 style = "position: absolute; left: 1em; bottom: -3em;",
                                 dropdown(
                ...
                tabPanel("Gene Normalized Counts",
                         fluidRow(column(6, div(
                             ...
                             div(
                                 style = "position: absolute; left: 1em; bottom: -0.7em;",
                                 dropdown(
                ...
                tabPanel("Splicing factors differential expression",
                         fluidRow(column(8, fluidRow(withSpinner(plotlyOutput("sf_deseq2"), type=4),
                                  div(
                                      style = "position: absolute; left: 1em; bottom: -2em;",
                                      dropdown(
        ...
        fluidRow(column(12,
            tabBox(
                ...
                tabPanel("Gene Switch plots",
                         ...
                         fluidRow(
                           div(style="padding-left: 20px;",
                            ...
                            actionButton("load_iso", "Visualize gene")
                            ),
                            div(withSpinner(
                ...
                tabPanel("Genes Switch table",
                         fluidRow(column(8,div(withSpinner(
    ...
    import_data_modal <- function(failed=F){
      if (failed==F){
          if (custom$custom==TRUE){
                modalDialog(
                    h4("Select gene column for RNA-seq table:"),
                    selectInput("rnaseq_genecol", "Select the gene column", choices=colnames(rnaseq_df())),
                    h4("Select sample column for meta table:"),
                    selectInput("rnaseq_metacol", "Select the sample names column", choices=colnames(rnameta_df())),
                    footer = tagList(
                        modalButton("Cancel"),
    ...
    rnaseq_df <- reactive({
        rnafile <- input$rnaseq_dir
        if (boris$boris==TRUE){
          df <- read_feather("examples/boris_gene_counts.feather")
        } else {
          # if (is.null(rnafile)) {
          #   showModal(import_data_modal(failed = TRUE))
          #   return(NULL)
          # }
          df <- make_gene_matrix(paste0(global$datapath,"/output/gene_counts/"))
        }
        return(df)
    }) 
    ...
    rnameta_df <- reactive({
        ...
        if (boris$boris==TRUE){
          ...
          #   return(NULL)
          # } 
          df <- read.csv(paste0(global$datapath,"/input/metadata.txt"), sep="\t", header=T)
        }
        return(df)
    }) 
    ...
    dds_obj <- reactive({
        if (boris$boris == TRUE){
            ...
            load(file=paste0(global$datapath,"/output/gene_counts/dds.RData"))
        }
        dds 
    })
    # 
    ...
    sf_deseq2_plot <- reactive({
        sf_genes<- read_feather("examples/splicing_factors_list.feather")
        
        if (boris$boris==TRUE){
            degs<-read_feather("examples/gene_counts/degs_deseq2.feather")
            ...
            degs<-read_feather(paste0(global$datapath, "/output/gene_counts/degs_deseq2.feather"))
            degs$ensembl_gene_id <- gsub(pattern = "\\..*$",replacement = "",x=degs$ensembl_gene_id_version)
            mart_export <-  mart_export_obj()
    ...
    isoform_data <- reactive({
        if (boris$boris == TRUE){
            ...
            load(file = paste0(global$datapath, "/output/pseudocounts/exampleSwitchListAnalyzed.RData"))
            datatable(extractSwitchSummary(exampleSwitchListAnalyzed))
        }
    ...
    exon_data <- reactive({
        if (boris$boris==TRUE){
            ...
            load(file = paste0(global$datapath, "/output/exon_counts/dxr1.RData"))
            return(dxr1)
        }
        return()
    })
    ...
    output$exons_table <- renderDataTable({voila_res_all() %>% dplyr::select(X.Gene.Name, Gene.ID, A5SS, A3SS, ES, IR.coords)})
   ...
   observe({
    ...
    voila_res_all <- reactive({
        if (boris$boris==TRUE){
            df<-read_feather( "examples/majiq_output/voila_results_all.feather")
        } else if (custom$custom==TRUE){
            df<-read_tsv(paste0(global$datapath, "/output/MAJIQ/voila_results_all.tsv"))
    ...
    voila_res <- reactive({
        if(boris$boris==TRUE){
            #df <- read.delim(file = "examples/MAJIQ/voila_results.tsv",header = TRUE,sep = "\t")
          df<-read_feather("examples/majiq_output/voila_results.feather")
        } else if (custom$custom==TRUE){
          df<-read_tsv(paste0(global$datapath, "/output/MAJIQ/voila_results.tsv"))
          colnames(df) <- lapply(colnames(df), function(x){gsub(" ", ".", x)})
          colnames(df) <- lapply(colnames(df), function(x){gsub("#", "X", x)})
        }
        return(df)
    ...
    observe({
        if (boris$boris==TRUE | custom$custom==TRUE) {
    ...
    star_qc <- reactive({
        if (boris$boris == TRUE){
            ...
            alignment_stats_star <- read_feather("examples/multiqc_data/multiqc_star.feather")
            alignment_stats_star <- reshape2::melt(data = alignment_stats_star,id.vars="Sample")#,"total_reads","avg_input_read_length"))
        } else if (custom$custom==TRUE){
          alignment_stats_star<-read_tsv(paste0(global$datapath, "/output/multiqc_data/multiqc_star.txt"))
          alignment_stats_star <- reshape2::melt(data = alignment_stats_star,id.vars="Sample")
        }
    ...
    pseudo_qc <- reactive({
        if (boris$boris == TRUE){
            ...
            alignment_stats_kallisto <- read_feather("examples/multiqc_data/multiqc_kallisto.feather")
            alignment_stats_kallisto <- reshape2::melt(data = alignment_stats_kallisto,id.vars="Sample")#,"total_reads","avg_input_read_length"))
        } else if (custom$custom==TRUE){
            alignment_stats_kallisto <-read_tsv(paste0(global$datapath, "/output/multiqc_data/multiqc_kallisto.txt"))
            alignment_stats_kallisto <- reshape2::melt(data = alignment_stats_kallisto,id.vars="Sample")
        }
    ...
    allchips_obj <- reactive({
        ...
        for (accession in 1:nrow(input_chips)) {
            #all.chips[[input_chips$Experiment.target[accession]]] <- purrr::reduce(readPeakFile(peakfile = paste0("examples/encode_bedNarrowPeak_files/",input_chips$File.accession[accession],".bed.gz")))
            all.chips[[input_chips$Experiment.target[accession]]] <- readPeakFile(peakfile = paste0("examples/encode_bedNarrowPeak_files/",input_chips$File.accession[accession],".bed.gz"))
        }
        
        #all.chips[[user_chip_name]] <- purrr::reduce(readPeakFile(peakfile = user_chip_file),)
    ...
    mart_export_obj <- reactive({
        ...
        mart_export <- read_feather("examples/mart_export.feather")
    ...
    output$iso_switch_plot_down <- downloadHandler(
        ...
        content = function(file) {
          ggsave(file, plot=switch_plotplot(), width = 10)
          unlink(file)})
