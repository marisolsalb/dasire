...
#exec 1>/MOUNT/DASiRe-preprocessing-$(date +%Y-%m-%d-%H.%M.%S).log 2>&1
#echo "$(date) DASiRe Preprocessing" >&3
# or
# Logging DASiRe's full run.
# () > /MOUNT/DASiRe-preprocessing-$(date +%Y-%m-%d-%H.%M.%S).log
